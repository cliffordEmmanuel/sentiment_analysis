...
if __name__ == '__main__':
    ...
    run_start = time.time()
    result = scrapeTweets(search_key,2,'2021-02-20')
    run_end = time.time()

    print(f"*******************************************************")
    print(f"Script took {round((run_end-run_start)/60,2)} mins to download {len(result.tweet_id)} tweets!!")
    print(f'''
    Number of unique tweets: {result.tweet_id.nunique()} 
    Number of duplicate tweets: {result.shape[0]-result.tweet_id.nunique()} 
    Dataframe size: {result.shape}
    Timestamp for oldest tweet downloaded: {result.tweet_created_at.min()}''')
    print(f"*******************************************************")